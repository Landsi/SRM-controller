...
PPTC hold current falls steeply with ambient temperature:
...
choice. Either the current model is pessimistic (it uses DC resistance and
...
**Keep the board out of engine-bay heat, and measure the actual supply current on
a built board.** That is the fix, not a bigger fuse.

The current model behind the figure: F1 sits upstream of Q4 and carries the whole
motor current; the firmware alternates two phases on and one phase on, 4 ms each,
giving a duty-weighted average of ~**11.3 A**, peak ~**17.8 A**, for **1.14 s**
per park/unpark.
